...
import { signInWithEmailAndPassword, createUserWithEmailAndPassword, signOut, signInWithPopup, GoogleAuthProvider, sendEmailVerification } from "https://www.gstatic.com/firebasejs/12.7.0/firebase-auth.js";
...
async function signup(event) {
  ...
  if (!emailRegex.test(email)) {
    customAlert.warning('Invalid Gmail', 'Please enter a valid Gmail address');
    return;
  }
  ...
  try {
    ...
    if (error.code === 'auth/email-already-in-use') {
      errorMessage = 'This Gmail is already registered. Please login instead.';
...
// Login function - Firebase with Gmail only
async function login(event) {
  ...
  // Validation
  if (!email || !password) {
    customAlert.warning('Missing Information', 'Please enter both Gmail and password');
    return;
  }
  ...
  try {
    ...
    // Email verification check - ENABLED for production security
    ...
    if (!user.emailVerified && !isSystemAccount) {
      ...
      const resend = await customAlert.confirm(
        'Gmail Not Verified',
        'Please verify your Gmail before logging in. Check your inbox (and spam folder) for the verification link.<br><br>Would you like us to resend the verification email?',
        'Resend Email',
        'Cancel'
      );
    ...
    let errorMessage = 'Invalid Gmail or password';

    if (error.code === 'auth/user-not-found') {
      errorMessage = 'No account found with this Gmail. Please sign up first.';
    } else if (error.code === 'auth/wrong-password') {
      errorMessage = 'Incorrect password. Please try again.';
    } else if (error.code === 'auth/invalid-credential') {
      errorMessage = 'Invalid Gmail or password. Please check and try again.';
    } else if (error.message) {
      errorMessage = error.message;
    }
...
async function handleGoogleSignIn() {
  try {
    const auth = getFirebaseAuth();
    if (!auth) {
      customAlert.error('Firebase Error', 'Firebase not initialized.');
      return;
    }
    ...
    provider.setCustomParameters({ 
      ...
      hd: '' // Allow any domain, not just specific ones
    });
    ...
    // Save/retrieve profile in Realtime Database
    const db = getFirebaseDatabase();
    if (db) {
      const userRef = ref(db, `users/${user.uid}`);
      const snapshot = await get(userRef);
      ...
      if (snapshot.exists()) {
        ...
        // Update email verification status for Google accounts
        if (user.emailVerified && !userProfile.emailVerified) {
          userProfile.emailVerified = true;
          await set(userRef, userProfile);
          console.log('✓ Updated email verification status in database');
        }
        ...
        const newProfile = {
          ...
          phone: '', // Can be filled later in profile settings
          address: '', // Can be filled later in profile settings
          gender: null,
          role: 'customer',
          warnings: 0,
          isBanned: false,
        ...
        console.log('✓ Creating new Google user profile:', newProfile);
        
        await set(userRef, newProfile);
        setCurrentUser(newProfile);
        console.log('✓ New Google user profile saved to database');
      }
    } else {
      ...
      if (existing) {
        ...
        // Hide loading before showing prompt
        if (typeof hideLoadingOverlay === 'function') hideLoadingOverlay();
    ...
    await customAlert.success('Welcome!', `Successfully signed in with Google as ${user.email}`);
